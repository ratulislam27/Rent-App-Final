...
function formatSignedMoney(value: number, symbol = "৳") {
  if (Math.abs(value) <= 0.01) return formatMoney(0, symbol);
...
function DocumentHeader({ title, id, date, dateLabel, status, balance, symbol }: { title: string; id: string; date: string; dateLabel: string; status: "paid" | "due" | "void"; balance?: number; symbol: string }) {
  ...
  return <header className="document-header"><div className="document-identity"><div className="document-title-line"><strong>{title}</strong><span>{id}</span></div><div className="document-date"><span>{dateLabel}</span><time dateTime={date}>{formatDate(date)}</time></div></div><div className={cn("document-status", `is-${status}`)}><strong>{label}</strong><span>{detail}</span></div></header>;
}
...
function RentBillDetail({ workspace, id, setDetail, openForm, service }: PageProps & { id: string }) {
  ...
  return <>
    ...
    <article className={cn("receipt-paper", bill.voided_at && "is-void", paymentHistory.length > 6 && "document-dense", paymentHistory.length > 12 && "document-extra-dense")}>
      ...
      <DocumentHeader title="Rent bill" id={bill.display_id} date={bill.issue_date} dateLabel="Issue date" status={bill.voided_at ? "void" : remaining <= 0.01 ? "paid" : "due"} balance={remaining} symbol={workspace.settings.currency_symbol} />
...
function ReceiptDetail({ workspace, id, setConfirm, service, refresh, notify }: PageProps & { id: string }) {
  ...
  const share = async () => {
    ...
    const due = relatedBills.length ? `\nCurrent balance due: ${formatMoney(balanceDue, workspace.settings.currency_symbol)}` : "";
  ...
  return <><PageHeading eyebrow={receipt.display_id} title="Rent receipt" subtitle={receipt.status === "void" ? "This receipt has been voided." : `Received ${formatDate(receipt.collection_date)}`} action={<div className="heading-actions"><button className="button button-secondary no-print" type="button" onClick={() => window.print()}><Printer size={16} />Print</button><button className="button button-primary no-print" type="button" onClick={() => void share()}><Share2 size={16} />Share</button></div>} />
    <article className={cn("receipt-paper", receipt.status === "void" && "is-void", allocations.length > 6 && "document-dense")}>
      ...
      <DocumentHeader title="Rent receipt" id={receipt.display_id} date={receipt.collection_date} dateLabel="Payment date" status={receipt.status === "void" ? "void" : balanceDue <= 0.01 ? "paid" : "due"} balance={balanceDue} symbol={workspace.settings.currency_symbol} />
      ...
      <div className="receipt-balance-summary"><div><span>Amount paid</span><strong>{formatMoney(amountPaid, workspace.settings.currency_symbol)}</strong></div><div className={balanceDue > 0 ? "has-balance" : "is-settled"}><span>Balance due</span><strong>{formatMoney(balanceDue, workspace.settings.currency_symbol)}</strong></div></div>
