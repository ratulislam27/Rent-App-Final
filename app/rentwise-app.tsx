...
import {
  ...
  receiptAllocations,
  rentBillPaid,
  rentBillPaymentHistory,
  rentBillRemaining,
  rentBillStatus,
  rentBillTotal,
  RentwiseDataService,
  validateDocumentSignatureFile,
} from "@/lib/data-service";
import { getSupabaseBrowserClient } from "@/lib/supabase";
import { formatTitleCase } from "@/lib/text";
import type {
  AdminUser,
  Agreement,
  Expense,
...
type FormKind = "property" | "tenant" | "agreement" | "collection" | "bill-charge" | "bill-charge-edit" | "expense" | "increment" | "profile" | "receipt-settings" | "lookup" | "password" | null;
...
function StatusLabel({ children, tone = "neutral" }: { children: ReactNode; tone?: "neutral" | "positive" | "warning" | "danger" | "info" }) {
  return <span className={cn("status-label", `status-${tone}`)}><span className="status-marker" aria-hidden="true" />{children}</span>;
}
...
function BillStatusLabel({ status }: { status: RentBillStatus }) {
  const tone = status === "paid" ? "positive" : status === "overdue" || status === "void" ? "danger" : status === "due" || status === "partially_paid" ? "warning" : "info";
  return <StatusLabel tone={tone}>{billStatusLabel(status)}</StatusLabel>;
}
...
function MutationSubmitButton({ children, pendingLabel = "Saving…", disabled = false, className = "button button-primary" }: { children: ReactNode; pendingLabel?: string; disabled?: boolean; className?: string }) {
  const pending = useContext(MutationPendingContext);
  return <button className={className} type="submit" disabled={disabled || pending} aria-busy={pending}>{pending ? <><LoaderCircle className="spin" size={16} />{pendingLabel}</> : children}</button>;
}

function MutationStatus() {
  return <div className="mutation-overlay" role="status" aria-live="polite" aria-label="Saving your changes">
    <div className="mutation-status"><LoaderCircle className="spin" size={20} /><span><strong>Saving your changes</strong><small>Please wait while we apply your updates…</small></span></div>
  </div>;
}

function useProfileImageUrl(service: RentwiseDataService, path: string | null) {
  ...
  useEffect(() => {
    ...
    void service.getProfileImageUrl(path).then((signedUrl) => { if (active) setUrl(signedUrl); }).catch(() => { if (active) setUrl(null); });
    return () => { active = false; };
  }, [path, service]);
  return url;
}

function ProfileAvatar({ name, path, service, className, previewUrl }: { name: string; path: string | null; service: RentwiseDataService; className?: string; previewUrl?: string | null }) {
  const storedUrl = useProfileImageUrl(service, path);
  const imageUrl = previewUrl || storedUrl;
  return <span className={cn("avatar", className, imageUrl && "avatar-has-image")} style={imageUrl ? { backgroundImage: `url(${JSON.stringify(imageUrl)})` } : undefined} aria-hidden="true">{imageUrl ? "" : initials(name)}</span>;
}

function ProfilePhotoPicker({ name, personName, currentPath, service }: { name: string; personName: string; currentPath: string | null; service: RentwiseDataService }) {
  ...
  return <div className="profile-photo-field">
    <ProfileAvatar name={personName} path={removed ? null : currentPath} service={service} className="avatar-photo-preview" previewUrl={previewUrl} />
    <div className="profile-photo-controls"><strong>Profile picture</strong><small>JPG, PNG or WebP · maximum 10 MB</small><div>
      <label className="button button-secondary profile-photo-picker"><Upload size={16} />{currentPath || previewUrl ? "Replace photo" : "Choose photo"}<input ref={inputRef} name={name} type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => { const file = event.target.files?.[0]; if (!file) return; if (previewUrl) URL.revokeObjectURL(previewUrl); setRemoved(false); setPreviewUrl(URL.createObjectURL(file)); }} /></label>
      {(currentPath || previewUrl) && !removed && <button className="text-button button-danger-text" type="button" onClick={() => { clearPreview(); setRemoved(true); }}>Remove</button>}
    </div></div>
...
function SignaturePicker({ currentPath, service }: { currentPath: string | null; service: RentwiseDataService }) {
  ...
  return <div className="signature-picker">
    <div className="signature-preview">{imageUrl ? <Image src={imageUrl} alt="Landlord signature preview" width={220} height={84} unoptimized /> : <span>Your signature preview</span>}</div>
    <div className="signature-picker-copy"><strong>Landlord signature</strong><small>JPG, PNG or WebP · maximum 10 MB. Use a tightly cropped image for the cleanest print.</small><div>
      <label className="button button-secondary profile-photo-picker"><Upload size={16} />{imageUrl ? "Replace signature" : "Upload signature"}<input ref={inputRef} name="signature" type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => { const file = event.target.files?.[0]; if (!file) return; try { validateDocumentSignatureFile(file); setFileError(""); } catch (reason) { clearPreview(); setFileError(reason instanceof Error ? reason.message : "This signature image cannot be used."); return; } if (previewUrl) URL.revokeObjectURL(previewUrl); setRemoved(false); setPreviewUrl(URL.createObjectURL(file)); }} /></label>
      {imageUrl && <button className="text-button button-danger-text" type="button" onClick={() => { clearPreview(); setRemoved(true); }}>Remove</button>}
    </div>{fileError && <span className="signature-file-error" role="alert"><CircleAlert size={14} />{fileError}</span>}</div>
...
function Sheet({ title, subtitle, children, onClose, wide = false, closeDisabled = false }: { title: string; subtitle?: string; children: ReactNode; onClose: () => void; wide?: boolean; closeDisabled?: boolean }) {
  const mutationPending = useContext(MutationPendingContext);
  const cannotClose = closeDisabled || mutationPending;
  return <div className="sheet-layer" role="presentation" onMouseDown={(event) => { if (!cannotClose && event.currentTarget === event.target) onClose(); }}>
    <section className={cn("sheet", wide && "sheet-wide")} role="dialog" aria-modal="true" aria-label={title}>
      <div className="sheet-grabber" />
      <header className="sheet-header"><div><h2>{title}</h2>{subtitle && <p>{subtitle}</p>}</div><button className="icon-button" type="button" onClick={onClose} aria-label="Close" disabled={cannotClose}><X size={19} /></button></header>
...
function ConfirmDialog({ title, text, confirmLabel, tone = "primary", onConfirm, onClose, inputLabel }: { title: string; text: string; confirmLabel: string; tone?: "primary" | "danger"; onConfirm: (value?: string) => void | Promise<void>; onClose: () => void; inputLabel?: string }) {
  ...
  return <Sheet title={title} subtitle={text} onClose={onClose} closeDisabled={busy}>
    {inputLabel && <Field label={inputLabel}><textarea value={value} onChange={(event) => setValue(event.target.value)} required /></Field>}
    <div className="sheet-actions"><button className="button button-secondary" type="button" onClick={onClose} disabled={busy}>Cancel</button><button className={cn("button", tone === "danger" ? "button-danger" : "button-primary")} type="button" disabled={busy || Boolean(inputLabel && !value.trim())} aria-busy={busy} onClick={async () => { setBusy(true); await onConfirm(value.trim()); setBusy(false); }}>{busy && <LoaderCircle className="spin" size={16} />}{confirmLabel}</button></div>
  </Sheet>;
}

function Toast({ message, tone = "success" }: { message: string; tone?: "success" | "error" }) {
  const Icon = tone === "error" ? CircleAlert : Check;
  return <div className={cn("toast", tone === "error" && "toast-error")} role={tone === "error" ? "alert" : "status"}><Icon size={16} />{message}</div>;
}
...
function AuthScreen({ service, theme, onToggleTheme, onDemo, onAuthenticated }: { service: RentwiseDataService; theme: ResolvedTheme; onToggleTheme: () => void; onDemo: () => void; onAuthenticated: (user: User) => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (mode === "register") {
        const fullName = formatTitleCase(String(form.get("name") || ""));
    ...
    <section className="auth-panel">
      <div className="auth-card">
        ...
        <form onSubmit={submit}>
          {mode === "register" && <Field label="Full name"><input name="name" autoComplete="name" autoCapitalize="words" required /></Field>}
...
export default function RentwiseApp() {
  ...
  const notify = useCallback((message: string) => { setErrorToast(""); setToast(message); window.setTimeout(() => setToast(""), 2600); }, []);
  const notifyError = useCallback((message: string) => { setToast(""); setErrorToast(message); window.setTimeout(() => setErrorToast(""), 4200); }, []);
  ...
  if (detail?.kind === "property") page = <PropertyDetail {...pageProps} id={detail.id} />;
    ...
    switch (route) {
      ...
      case "more": page = <MorePage workspace={workspace} service={service} navigate={navigate} signOut={signOut} />; break;
      default: page = <HomePage {...pageProps} />;
    }
  }

  return <MutationPendingContext.Provider value={mutationPending}><div className="app-shell" aria-busy={mutationPending}>
    ...
    {form === "bill-charge-edit" && formTarget && <RentBillChargeForm workspace={workspace} service={service} chargeId={formTarget} onClose={closeForm} mutate={mutate} />}
    ...
    {confirm && <ConfirmDialog title={confirm.title} text={confirm.text} confirmLabel={confirm.label} tone={confirm.tone} inputLabel={confirm.inputLabel} onClose={() => setConfirm(null)} onConfirm={async (value) => { try { await confirm.action(value); setConfirm(null); } catch (reason) { const message = reason instanceof Error ? reason.message : "The action could not be completed."; setError(message); notifyError(message); } }} />}
    {mutationPending && <MutationStatus />}
    {errorToast && <Toast message={errorToast} tone="error" />}
    {toast && <Toast message={toast} />}
  </div></MutationPendingContext.Provider>;
}
...
function PageHeading({ eyebrow, title, subtitle, action, leading }: { eyebrow?: string; title: string; subtitle?: string; action?: ReactNode; leading?: ReactNode }) {
  return <div className="page-heading"><div className={cn("page-heading-title", Boolean(leading) && "has-leading")}>{leading}<div className="page-heading-copy">{eyebrow && <p className="eyebrow">{eyebrow}</p>}<h1>{title}</h1>{subtitle && <p>{subtitle}</p>}</div></div>{action}</div>;
}
...
function MorePage({ workspace, service, navigate, signOut }: { workspace: WorkspaceData; service: RentwiseDataService; navigate: (route: Route) => void; signOut: () => void }) {
...
function HomePage(props: PageProps) {
  ...
  return <>
    ...
    <div className="metric-grid">
      ...
      <button type="button" className="metric metric-tone metric-vacant" onClick={() => navigate("properties")}><span>Vacant</span><strong>{vacant}</strong><small>properties</small></button>
      <button type="button" className="metric metric-tone metric-expenses" onClick={() => navigate("expenses")}><span>Expenses</span><strong>{formatMoney(expenses, symbol)}</strong><small>this month</small></button>
    </div>
...
function PropertiesPage({ workspace, setDetail, openForm }: PageProps) {
  ...
  return <><PageHeading eyebrow={`${workspace.properties.filter((item) => !item.archived_at).length} active records`} title="Properties" subtitle="Each record represents one rentable unit." action={<button className="button button-primary" type="button" onClick={() => openForm("property")}><Plus size={17} />Add property</button>} />
    ...
    <div className="record-list">{rows.map((property) => {
      ...
      return <button className="record-row" type="button" key={property.id} onClick={() => setDetail({ kind: "property", id: property.id })}><IconTile icon={Icon} /><span className="record-main"><strong>{property.name}</strong><small>{property.display_id} · {agreement ? tenant?.name : property.location}</small></span><StatusLabel tone={property.status === "occupied" ? "positive" : property.status === "maintenance" ? "danger" : "warning"}>{property.status[0].toUpperCase() + property.status.slice(1)}</StatusLabel><ChevronRight className="row-chevron" size={17} /></button>;
...
function TenantsPage({ workspace, service, setDetail, openForm }: PageProps) {
  const [query, setQuery] = useState("");
...
function TenantDetail({ workspace, id, setDetail, openForm, setConfirm, service, refresh, notify }: PageProps & { id: string }) {
  ...
  return <><PageHeading eyebrow={tenant.display_id} title={tenant.name} subtitle={tenant.phone} leading={<ProfileAvatar name={tenant.name} path={tenant.profile_image_path} service={service} className="avatar-page" />} action={<div className="heading-actions"><a className="button button-secondary" href={`tel:${tenant.phone}`}><Phone size={16} />Call</a><button className="button button-secondary" type="button" onClick={() => openForm("tenant", tenant.id)}><Pencil size={16} />Edit</button></div>} />
...
function AgreementDetail({ workspace, id, setDetail, openForm, setConfirm, service, refresh, notify }: PageProps & { id: string }) {
  ...
  return <><PageHeading eyebrow={agreement.display_id} title={`${tenant?.name} · ${property?.name}`} subtitle={`${formatDate(agreement.start_date)} – ${formatDate(agreement.end_date)}`} action={<div className="heading-actions"><button className="button button-secondary" type="button" onClick={() => openForm("agreement", agreement.id)}><Pencil size={16} />Edit</button><button className="button button-primary" type="button" onClick={() => openForm("collection", agreement.id)}><WalletCards size={16} />Receive payment</button></div>} />
    <div className="detail-grid"><section className="detail-card"><SectionHeading title="Agreement terms" /><dl className="detail-list"><div><dt>Status</dt><dd><StatusLabel tone={status === "active" ? "positive" : status === "upcoming" ? "info" : "neutral"}>{status}</StatusLabel></dd></div><div><dt>Monthly base rent</dt><dd>{formatMoney(agreement.monthly_base_rent, workspace.settings.currency_symbol)}</dd></div><div><dt>Security deposit</dt><dd>{formatMoney(agreement.security_deposit, workspace.settings.currency_symbol)}</dd></div><div><dt>Notice period</dt><dd>{agreement.notice_period_months} month{agreement.notice_period_months === 1 ? "" : "s"}</dd></div></dl></section>
...
function RentBillDetail({ workspace, id, setDetail, openForm, service }: PageProps & { id: string }) {
  ...
  return <>
    ...
    <article className={cn("receipt-paper", bill.voided_at && "is-void", paymentHistory.length > 6 && "document-dense", paymentHistory.length > 12 && "document-extra-dense")}>
      ...
      <DocumentHeader title={`${monthLabel(bill.rent_month)} rent bill`} id={bill.display_id} status={bill.voided_at ? "void" : remaining <= 0.01 ? "paid" : "due"} balance={remaining} symbol={workspace.settings.currency_symbol} />
      ...
      <div className="receipt-lines"><div><span>Issue date</span><strong>{formatDate(bill.issue_date)}</strong></div><div><span>Due date</span><strong>{formatDate(bill.due_date)}</strong></div><div><span>Base rent</span><strong>{formatMoney(bill.base_rent, workspace.settings.currency_symbol)}</strong></div>{charges.map((charge) => <div className="bill-charge-line" key={charge.id}><span>{charge.reason}</span><span className="bill-charge-value"><strong>{formatMoney(charge.amount, workspace.settings.currency_symbol)}</strong>{!bill.voided_at && <button className="bill-charge-edit no-print" type="button" aria-label={`Edit ${charge.reason} charge`} title="Edit charge" onClick={() => openForm("bill-charge-edit", charge.id)}><Pencil size={13} /></button>}</span></div>)}</div>
      <div className="invoice-balance-summary"><div><span>Bill total</span><strong>{formatMoney(total, workspace.settings.currency_symbol)}</strong></div><div><span>Amount paid</span><strong className="positive-value">{formatMoney(paid, workspace.settings.currency_symbol)}</strong></div><div className={remaining > 0 ? "has-balance" : "is-settled"}><span>Amount due</span><strong>{formatMoney(remaining, workspace.settings.currency_symbol)}</strong></div></div>
      <section className="invoice-payments"><div className="invoice-section-heading"><div><strong>Payment history</strong><span>Payment activity for this bill</span></div><span>{validPayments.length} payment{validPayments.length === 1 ? "" : "s"}</span></div>{paymentHistory.length ? <div className={cn("invoice-payment-list", !validPayments.length && "has-no-valid-payments")}><div className="invoice-payment-columns" aria-hidden="true"><span>Date</span><span>Receipt</span><span>Method</span><span>Payment</span><span>Amount left</span></div>{paymentHistory.map(({ allocation, receipt, paymentMethod, balanceAfter }) => <button className={cn("invoice-payment-row", receipt.status === "void" && "is-void")} type="button" key={allocation.id} onClick={() => setDetail({ kind: "receipt", id: receipt.id })}><span className="invoice-payment-date">{formatDate(receipt.collection_date)}</span><span className="invoice-payment-reference"><strong>{receipt.display_id}</strong>{receipt.status === "void" && <small>Void</small>}</span><span className="invoice-payment-method">{paymentMethod?.name ?? "Not recorded"}</span><span className="invoice-payment-amount"><strong>{formatMoney(allocation.allocated_amount, workspace.settings.currency_symbol)}</strong>{receipt.status === "void" && <small>Not counted</small>}</span><span className={cn("invoice-payment-balance", balanceAfter > 0.01 ? "is-owed" : balanceAfter < -0.01 ? "is-credit" : "is-settled")}><strong>{formatMoney(Math.abs(balanceAfter), workspace.settings.currency_symbol)}</strong><small>{balanceAfter > 0.01 ? "still due" : balanceAfter < -0.01 ? "tenant credit" : "paid in full"}</small></span><ChevronRight className="no-print" size={15} /></button>)}{!validPayments.length && <div className="invoice-payment-print-empty">No valid payments are recorded for this bill.</div>}</div> : <><div className="invoice-payment-empty no-print"><WalletCards size={20} /><div><strong>No payments received</strong><span>Payments will appear here as soon as a receipt is allocated to this bill.</span></div></div><div className="document-empty-print">No payments are recorded for this bill.</div></>}</section>
...
function ReceiptDetail({ workspace, id, setConfirm, service, refresh, notify }: PageProps & { id: string }) {
  ...
  const relatedBills = Array.from(new Map(allocations.map(({ bill }) => [bill!.id, bill!])).values());
  const balanceDue = relatedBills.reduce((sum, bill) => {
    const paymentSnapshot = rentBillPaymentHistory(workspace, bill).find((entry) => entry.receipt.id === receipt.id);
    return sum + (paymentSnapshot?.balanceAfter ?? rentBillRemaining(workspace, bill));
  }, 0);
  const amountPaid = receipt.status === "valid" ? receipt.amount : 0;
  const share = async () => {
    ...
    const due = relatedBills.length ? `\nCurrent amount due: ${formatMoney(balanceDue, workspace.settings.currency_symbol)}` : "";
    const text = `Rent receipt ${receipt.display_id}\n${tenant?.name} · ${property?.name}\n${applied}${credit}\nAmount received: ${formatMoney(receipt.amount, workspace.settings.currency_symbol)}${due}`;
  ...
  return <><PageHeading eyebrow={receipt.display_id} title="Rent receipt" subtitle={receipt.status === "void" ? "This receipt has been voided." : `Received ${formatDate(receipt.collection_date)}`} action={<div className="heading-actions"><button className="button button-secondary no-print" type="button" onClick={() => window.print()}><Printer size={16} />Print</button><button className="button button-primary no-print" type="button" onClick={() => void share()}><Share2 size={16} />Share</button></div>} />
    <article className={cn("receipt-paper", receipt.status === "void" && "is-void", allocations.length > 6 && "document-dense")}>
      {receipt.status === "void" && <div className="void-watermark">VOID</div>}
      <DocumentHeader title="Rent receipt" id={receipt.display_id} status={receipt.status === "void" ? "void" : balanceDue <= 0.01 ? "paid" : "due"} balance={balanceDue} symbol={workspace.settings.currency_symbol} />
      ...
      <section className="receipt-application"><div className="document-section-title"><strong>Payment details</strong><span>Where this payment was applied</span></div><div className="receipt-lines"><div><span>Payment date</span><strong>{formatDate(receipt.collection_date)}</strong></div>{allocations.map(({ allocation, bill }) => <div key={allocation.id}><span>{monthLabel(bill!.rent_month)} · {bill!.display_id}</span><strong>{formatMoney(allocation.allocated_amount, workspace.settings.currency_symbol)}</strong></div>)}{receipt.unallocated_amount > 0 && <div><span>Tenant credit</span><strong>{formatMoney(receipt.unallocated_amount, workspace.settings.currency_symbol)}</strong></div>}<div><span>Payment method</span><strong>{method?.name ?? "Not recorded"}</strong></div></div></section>
      <div className="receipt-balance-summary"><div><span>Amount paid</span><strong>{formatMoney(amountPaid, workspace.settings.currency_symbol)}</strong></div><div className={balanceDue > 0 ? "has-balance" : "is-settled"}><span>Amount due</span><strong>{formatMoney(balanceDue, workspace.settings.currency_symbol)}</strong></div></div>
      {receipt.notes && <div className="document-note"><span>Note</span><p>{receipt.notes}</p></div>}
      <DocumentSignatures workspace={workspace} service={service} />
      <footer className="receipt-footer"><span>{workspace.settings.receipt_phone}</span><span>{workspace.settings.receipt_address}</span></footer>
    </article>
...
function ExpenseDetail({ workspace, id, setDetail }: PageProps & { id: string }) {
  ...
  return <>
    ...
    <div className="detail-grid">
      <section className="detail-card"><SectionHeading title="Expense details" /><dl className="detail-list"><div><dt>Amount</dt><dd>{formatMoney(expense.amount, workspace.settings.currency_symbol)}</dd></div><div><dt>Category</dt><dd>{category}</dd></div><div><dt>Status</dt><dd><StatusLabel tone={expense.status === "valid" ? "positive" : "danger"}>{expense.status}</StatusLabel></dd></div><div><dt>Notes</dt><dd>{expense.notes || "No internal notes"}</dd></div></dl></section>
...
function ReportsPage({ workspace }: PageProps) {
  ...
  return <><PageHeading eyebrow="Printable records" title="Reports" subtitle="Filter the information you need, then print or save it as a PDF." action={<div className="heading-actions no-print"><button className="button button-secondary" type="button" onClick={() => window.print()}><Printer size={16} />Print</button><button className="button button-primary" type="button" onClick={() => window.print()}><Download size={16} />Save PDF</button></div>} />
    ...
    <article className="report-paper"><header className="report-header"><div><span className="eyebrow">Rento report</span><h2>{titleMap[report]}</h2><p>{periodLabel} · Generated {formatDate(todayISO)}</p></div><div className="report-owner"><strong>Rento</strong><span>{workspace.settings.receipt_phone}</span></div></header>
  ...
  return <>
    ...
    <div className="settings-grid">
      ...
      <section className="settings-section"><SectionHeading title="Profile" /><div className="menu-list">
        <button className="menu-row" type="button" onClick={() => openForm("profile")}><ProfileAvatar name={workspace.profile.full_name} path={workspace.profile.avatar_path} service={service} /><span><strong>Personal information</strong><small>{workspace.profile.full_name} · {workspace.profile.email}</small></span><ChevronRight size={17} /></button>
        ...
        <button className="menu-row" type="button" onClick={() => openForm("receipt-settings")}><IconTile icon={ReceiptText} /><span><strong>Document settings</strong><small>Contact details, currency and signature</small></span><ChevronRight size={17} /></button>
      </div></section>
...
function AdminPage({ workspace, service, client, notify }: PageProps & { client: ReturnType<typeof getSupabaseBrowserClient> }) {
  ...
  const [actionPending, setActionPending] = useState<"deactivate" | "reactivate" | "reset-password" | null>(null);
  ...
  async function runAction(user: AdminUser, action: "deactivate" | "reactivate" | "reset-password") {
    ...
    try {
      ...
      if (!session) throw new Error("Your administrator session is no longer available.");
  ...
  return <><PageHeading eyebrow="Platform administration" title="Landlord accounts" subtitle="Manage access and audited support operations. Existing passwords are never visible." />
    ...
    <div className="record-list">{loading ? <div className="list-loading"><LoaderCircle className="spin" size={20} />Loading accounts</div> : filtered.filter((item) => !item.is_admin).map((account) => <button className="record-row" type="button" key={account.id} onClick={() => setSelected(account)}><ProfileAvatar name={account.full_name} path={account.avatar_path} service={service} /><span className="record-main"><strong>{account.full_name}</strong><small>{account.email} · joined {formatDate(account.created_at)}</small></span><StatusLabel tone={account.is_active ? "positive" : "danger"}>{account.is_active ? "Active" : "Inactive"}</StatusLabel><ChevronRight size={17} /></button>)}</div>
    {selected && <Sheet title={selected.full_name} subtitle={selected.email} onClose={() => setSelected(null)} closeDisabled={Boolean(actionPending)}><dl className="detail-list"><div><dt>Account ID</dt><dd className="mono-value">{selected.id}</dd></div><div><dt>Status</dt><dd>{selected.is_active ? "Active" : "Inactive"}</dd></div><div><dt>Joined</dt><dd>{formatDate(selected.created_at)}</dd></div></dl>{actionError && <div className="inline-alert inline-alert-danger"><CircleAlert size={17} />{actionError}</div>}<div className="admin-actions"><Field label="Temporary password" hint="The landlord must choose a new password after signing in."><input type="text" value={temporaryPassword} onChange={(event) => setTemporaryPassword(event.target.value)} minLength={8} placeholder="At least 8 characters" /></Field><button className="button button-secondary button-block" type="button" disabled={temporaryPassword.length < 8 || Boolean(actionPending)} aria-busy={actionPending === "reset-password"} onClick={() => void runAction(selected, "reset-password")}>{actionPending === "reset-password" ? <LoaderCircle className="spin" size={16} /> : <KeyRound size={16} />}{actionPending === "reset-password" ? "Resetting…" : "Reset password"}</button><button className={cn("button button-block", selected.is_active ? "button-danger" : "button-primary")} type="button" disabled={Boolean(actionPending)} aria-busy={actionPending === (selected.is_active ? "deactivate" : "reactivate")} onClick={() => void runAction(selected, selected.is_active ? "deactivate" : "reactivate")}>{actionPending === (selected.is_active ? "deactivate" : "reactivate") ? <LoaderCircle className="spin" size={16} /> : selected.is_active ? <Ban size={16} /> : <RefreshCw size={16} />}{actionPending === (selected.is_active ? "deactivate" : "reactivate") ? "Updating…" : selected.is_active ? "Deactivate account" : "Reactivate account"}</button></div></Sheet>}
  </>;
}
...
function TenantForm({ workspace, service, targetId, onClose, mutate }: { workspace: WorkspaceData; service: RentwiseDataService; targetId: string | null; onClose: () => void; mutate: (operation: () => Promise<unknown>, success: string) => Promise<void> }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    await mutate(async () => {
      const record = existing ? (await service.updateTenant(existing.id, values), existing) : await service.createTenant(workspace.profile.id, values);
      const photo = (element.elements.namedItem("profilePhoto") as HTMLInputElement)?.files?.[0];
      const removePhoto = form.get("profilePhotoRemove") === "true";
      if (photo) await service.replaceProfileImage(workspace.profile.id, "tenant", record.id, photo, record.profile_image_path);
      else if (removePhoto) await service.removeProfileImage("tenant", record.id, record.profile_image_path);
      const files = Array.from((element.elements.namedItem("attachments") as HTMLInputElement)?.files ?? []);
      for (const file of files) await service.uploadAttachment(workspace.profile.id, "tenant", record.id, file);
    }, existing ? `${existing.display_id} updated` : "Tenant created with a new ID");
  }
  return <Sheet title={existing ? "Edit tenant" : "Add tenant"} subtitle={existing ? existing.display_id : "A permanent tenant ID is generated automatically."} onClose={onClose}><form onSubmit={submit}><ProfilePhotoPicker name="profilePhoto" personName={existing?.name ?? "New tenant"} currentPath={existing?.profile_image_path ?? null} service={service} /><Field label="Full name"><input name="name" autoCapitalize="words" defaultValue={existing?.name} required /></Field><div className="field-grid"><Field label="Phone"><input name="phone" type="tel" defaultValue={existing?.phone} required /></Field><Field label="Email (optional)"><input name="email" type="email" defaultValue={existing?.email ?? ""} /></Field></div><Field label="Address"><textarea name="address" defaultValue={existing?.address} /></Field><Field label="NID (optional)" hint="NID is masked in ordinary views."><input name="nid" inputMode="numeric" defaultValue={existing?.nid ?? ""} /></Field><Field label="Other attachments" hint="JPG, PNG, WebP or PDF. Maximum 10 MB per file."><input name="attachments" type="file" accept="image/jpeg,image/png,image/webp,application/pdf" multiple /></Field><Field label="Internal notes"><textarea name="notes" defaultValue={existing?.notes} /></Field><div className="sheet-actions"><button className="button button-secondary" type="button" onClick={onClose}>Cancel</button><MutationSubmitButton pendingLabel={existing ? "Saving…" : "Creating…"}>{existing ? "Save changes" : "Create tenant"}</MutationSubmitButton></div></form></Sheet>;
}
...
function RentBillChargeForm({ workspace, service, billId, chargeId, onClose, mutate }: { workspace: WorkspaceData; service: RentwiseDataService; billId?: string; chargeId?: string; onClose: () => void; mutate: (operation: () => Promise<unknown>, success: string) => Promise<void> }) {
  ...
  return <Sheet title={charge ? "Edit bill charge" : "Add bill charge"} subtitle={`${bill.display_id} · ${monthLabel(bill.rent_month)}`} onClose={onClose}><form onSubmit={submit}><Field label="Charge reason"><input name="reason" defaultValue={charge?.reason} required /></Field><Field label="Amount" hint={amountHint}><input name="amount" type="number" inputMode="decimal" min={charge ? minimumAmount : 0.01} step="0.01" defaultValue={charge?.amount} required /></Field><div className="sheet-actions"><button className="button button-secondary" type="button" onClick={onClose}>Cancel</button><MutationSubmitButton pendingLabel={charge ? "Saving…" : "Adding…"}>{charge ? "Save changes" : "Add charge"}</MutationSubmitButton></div></form></Sheet>;
}
...
function ProfileForm({ workspace, service, onClose, mutate }: { workspace: WorkspaceData; service: RentwiseDataService; onClose: () => void; mutate: (operation: () => Promise<unknown>, success: string) => Promise<void> }) {
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); const element = event.currentTarget; const form = new FormData(element);
    await mutate(async () => {
      await service.updateProfile(workspace.profile.id, { full_name: String(form.get("name") || "").trim(), phone: String(form.get("phone") || "").trim(), address: String(form.get("address") || "").trim() });
      const photo = (element.elements.namedItem("profilePhoto") as HTMLInputElement)?.files?.[0];
      if (photo) await service.replaceProfileImage(workspace.profile.id, "profile", workspace.profile.id, photo, workspace.profile.avatar_path);
      else if (form.get("profilePhotoRemove") === "true") await service.removeProfileImage("profile", workspace.profile.id, workspace.profile.avatar_path);
    }, "Profile updated");
  }
  return <Sheet title="Personal information" subtitle="Your email is managed by your sign-in account." onClose={onClose}><form onSubmit={submit}><ProfilePhotoPicker name="profilePhoto" personName={workspace.profile.full_name} currentPath={workspace.profile.avatar_path} service={service} /><Field label="Full name"><input name="name" autoCapitalize="words" defaultValue={workspace.profile.full_name} required /></Field><Field label="Email"><input value={workspace.profile.email} disabled /></Field><Field label="Phone"><input name="phone" type="tel" defaultValue={workspace.profile.phone} /></Field><Field label="Address"><textarea name="address" defaultValue={workspace.profile.address} /></Field><div className="sheet-actions"><button className="button button-secondary" type="button" onClick={onClose}>Cancel</button><MutationSubmitButton>Save changes</MutationSubmitButton></div></form></Sheet>;
}
...
function ReceiptSettingsForm({ workspace, service, onClose, mutate }: { workspace: WorkspaceData; service: RentwiseDataService; onClose: () => void; mutate: (operation: () => Promise<unknown>, success: string) => Promise<void> }) {
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); const element = event.currentTarget; const form = new FormData(element);
    await mutate(async () => {
      const signature = (element.elements.namedItem("signature") as HTMLInputElement)?.files?.[0];
      await service.saveDocumentSettings(
        workspace.profile.id,
        { receipt_phone: String(form.get("phone") || "").trim(), receipt_address: String(form.get("address") || "").trim(), currency_code: String(form.get("currencyCode") || "BDT"), currency_symbol: String(form.get("currencySymbol") || "৳"), timezone: String(form.get("timezone") || "Asia/Dhaka") },
        signature ?? null,
        workspace.settings.signature_path ?? null,
        form.get("signatureRemove") === "true",
      );
    }, "Document settings updated");
  }
  return <Sheet title="Document settings" subtitle="These details appear on tenant-facing bills, receipts and print layouts." onClose={onClose}><form onSubmit={submit}><SignaturePicker currentPath={workspace.settings.signature_path ?? null} service={service} /><Field label="Contact phone"><input name="phone" type="tel" defaultValue={workspace.settings.receipt_phone} /></Field><Field label="Contact address"><textarea name="address" defaultValue={workspace.settings.receipt_address} /></Field><div className="field-grid"><Field label="Currency code"><input name="currencyCode" defaultValue={workspace.settings.currency_code} maxLength={3} required /></Field><Field label="Currency symbol"><input name="currencySymbol" defaultValue={workspace.settings.currency_symbol} maxLength={4} required /></Field></div><Field label="Time zone"><input name="timezone" defaultValue={workspace.settings.timezone} required /></Field><div className="sheet-actions"><button className="button button-secondary" type="button" onClick={onClose}>Cancel</button><MutationSubmitButton>Save settings</MutationSubmitButton></div></form></Sheet>;
}
...
function LookupManager({ workspace, service, table, onClose, mutate }: { workspace: WorkspaceData; service: RentwiseDataService; table: "property_types" | "payment_methods" | "expense_categories"; onClose: () => void; mutate: (operation: () => Promise<unknown>, success: string) => Promise<void> }) {
  ...
  async function add(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const form = new FormData(event.currentTarget); const name = formatTitleCase(String(form.get("name") || "")); if (!name) return; await mutate(() => service.addLookup(table, workspace.profile.id, name), `${name} added`); }
  return <Sheet title={title} subtitle="Used options are deactivated instead of deleted, preserving history." onClose={onClose}><form className="inline-add" onSubmit={add}><input name="name" autoCapitalize="words" placeholder={`Add ${title.toLowerCase().slice(0, -1)}`} required /><MutationSubmitButton pendingLabel="Adding…"><Plus size={16} />Add</MutationSubmitButton></form><div className="lookup-list">{list.map((item) => <div key={item.id}><span>{item.name}</span><button className="button button-small button-secondary" type="button" onClick={() => void mutate(() => service.toggleLookup(table, item.id, !item.is_active), `${item.name} ${item.is_active ? "deactivated" : "activated"}`)}>{item.is_active ? "Deactivate" : "Activate"}</button></div>)}</div></Sheet>;
}
...
function PasswordForm({ client, onChanged, onClose, notify }: { client: ReturnType<typeof getSupabaseBrowserClient>; onChanged: () => Promise<void>; onClose: () => void; notify: (message: string) => void }) {
  ...
  return <Sheet title="Change password" subtitle="Enter your current password before choosing a new one." onClose={onClose} closeDisabled={busy}><form onSubmit={submit}><Field label="Current password"><input name="current" type="password" autoComplete="current-password" required /></Field><Field label="New password"><input name="password" type="password" minLength={8} autoComplete="new-password" required /></Field><Field label="Confirm new password"><input name="confirmation" type="password" minLength={8} autoComplete="new-password" required /></Field>{error && <div className="inline-alert inline-alert-danger"><CircleAlert size={17} />{error}</div>}<div className="sheet-actions"><button className="button button-secondary" type="button" onClick={onClose} disabled={busy}>Cancel</button><button className="button button-primary" type="submit" disabled={busy} aria-busy={busy}>{busy && <LoaderCircle className="spin" size={16} />}{busy ? "Updating…" : "Update password"}</button></div></form></Sheet>;
}
