...
function RentBillDetail({ workspace, id, setDetail, openForm }: PageProps & { id: string }) {
  ...
  const validInstallments = paymentHistory.filter((entry) => entry.receipt.status === "valid");
  const settledBy = validInstallments.findLast((entry) => entry.balanceAfter <= 0.01)?.receipt.collection_date;
  return <>
