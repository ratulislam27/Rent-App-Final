...
function ProfileForm({ workspace, service, onClose, mutate }: { workspace: WorkspaceData; service: RentwiseDataService; onClose: () => void; mutate: (operation: () => Promise<unknown>, success: string) => Promise<void> }) {
  ...
  return <Sheet title="Personal information" subtitle="Your email is managed by your sign-in account." onClose={onClose}><form onSubmit={submit}><ProfilePhotoPicker name="profilePhoto" personName={workspace.profile.full_name} currentPath={workspace.profile.avatar_path} service={service} /><Field label="Full name"><input name="name" defaultValue={workspace.profile.full_name} required /></Field><Field label="Email"><input value={workspace.profile.email} disabled /></Field><Field label="Phone"><input name="phone" type="tel" defaultValue={workspace.profile.phone} /></Field><Field label="Address"><textarea name="address" defaultValue={workspace.profile.address} /></Field><div className="sheet-actions"><button className="button button-secondary" type="button" onClick={onClose}>Cancel</button><button className="button button-primary" type="submit">Save changes</button></div></form></Sheet>;
}
...
function ReceiptSettingsForm({ workspace, service, onClose, mutate }: { workspace: WorkspaceData; service: RentwiseDataService; onClose: () => void; mutate: (operation: () => Promise<unknown>, success: string) => Promise<void> }) {
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); const element = event.currentTarget; const form = new FormData(element);
    await mutate(async () => {
      await service.updateSettings(workspace.profile.id, { receipt_phone: String(form.get("phone") || "").trim(), receipt_address: String(form.get("address") || "").trim(), currency_code: String(form.get("currencyCode") || "BDT"), currency_symbol: String(form.get("currencySymbol") || "৳"), timezone: String(form.get("timezone") || "Asia/Dhaka") });
      const signature = (element.elements.namedItem("signature") as HTMLInputElement)?.files?.[0];
      if (signature) await service.replaceDocumentSignature(workspace.profile.id, signature, workspace.settings.signature_path ?? null);
      else if (form.get("signatureRemove") === "true") await service.removeDocumentSignature(workspace.profile.id, workspace.settings.signature_path ?? null);
